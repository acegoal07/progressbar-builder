 ...
 */
module.exports = ProgressBar = ({end = null, position = null, size = 40, line = "▢", slider = "▣", hideInfo = false, percentage = false}) => {
   // Error messages
   ...
   // Progress bar output
   const barInfo = !percentage ? (position+' / '+end) : `${percentage * 100}%`;
   return `[${progressText+emptyProgressText}] `+`${hideInfo ? barInfo : ""}`;
}